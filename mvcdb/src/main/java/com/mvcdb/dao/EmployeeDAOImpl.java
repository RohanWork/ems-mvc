...
//	}
	...
	String sqlInsertAudit  = "INSERT INTO "+auditTable+" (id, name, gender, age, email, address, telephone, department, password, salary, rowInsTms, rowDelTms, status, token, modifier)"
		    			   + "VALUES (:id, :name, :gender, :age, :email, :address, :telephone, :department, :password, :salary, :rowInsTms, :rowDelTms, :status, :token, :modifier)";
	...
	String sqlEmployeeByMailMain  = "SELECT * FROM "+mainTable+" WHERE email=:email";
...
//		});
	...
	public MapSqlParameterSource getParam(Employee employee) {
		...
		source.addValue("modifier", employee.getModifier());
		return source;
		...
		List<Employee> auditEmployee = jdbcTemplate.query(sqlAuditList, new ResultSetExtractor<List<Employee>>() {
			...
			public List<Employee> extractData(ResultSet rs) throws SQLException, DataAccessException {
				while(rs.next()) {
					...
					employee.setStatus(rs.getString("status"));
					employee.setToken(rs.getString("token"));
					employee.setModifier(rs.getString("modifier"));
					auditData.add(employee);
				}
				return auditData;
			}
		});
		List<Employee> sortedEmployees = auditEmployee.stream()
	...
	public Employee getUserByEmail(String email) {
		...
		return jdbcTemplate.query(sqlEmployeeByMailMain, source, new ResultSetExtractor<Employee>() {
